...
$ ./bicep.exe --version
Bicep CLI version 0.47.100 (dab88b470e)
```
...
The body keys are what the author wrote, with this case's values: parameters, variables, loop items,
conditional expressions and other declarations' names and IDs are all worked out. They are not what
Azure would report back. A value Azure assigns when it creates something — a principal ID, an IP
address, a provisioning state — does not exist offline. `dependsOn` and the internals of a module's
deployment are generated by the compiler and are not exposed; a module's resources are reached through
`withModules`. A `Microsoft.Resources/deployments` resource you declare yourself is not a module, so
its `properties` are reported like any other body, with its inline `template` exactly as written.

`fleet.biceptest` checks where each data account is deployed, and that each stamp's site runs on the
plan of its own stamp — a resource ID computed from another declaration, inside a module:

```bicep
eachAccountDeploysToItsRegion: {
  passWhen: map(filter(target.evaluated.resources, r => r.symbolicName == 'dataAccounts'), r => r.location) == regions
  message: 'Each data account must be deployed to the region it was created for.'
}

eachSiteRunsOnItsOwnPlan: {
  failOn: filter(target.evaluated.withModules.resources, r => r.type == 'Microsoft.Web/sites' && !endsWith(r.properties.serverFarmId, '/${replace(r.name, '-site', '-plan')}'))
  message: 'Each site must run on the plan of its own stamp.'
}
```

Body values are derived from parameters and mock responses, so they are never written to a report. A
`failOn` that returns instances reports each one by its declaration and name (see "Violations name
the instance and the declaration" below), however much of its body the assertion read.

Bodies are computed only for an assertion that reads one, and they are all-or-nothing for the
instances in scope: if any instance's body could not be computed, reading a body fails and names it.

```text
Could not be evaluated: The properties of 'assignment' (main.bicep(6)) could not be evaluated for this case: The language expression property 'principalId' doesn't exist, ...
```
...
[✗] Evaluation fleet-failing.biceptest: storageOnly (fleet.bicep) Failed at 1 / 1 assertions!
	[✗] Assertion onlyStorageIsDeployed failed!
		...
		fleet/regionStamp.bicep(16): fleetdev-secondary-plan
		fleet/regionStamp.bicep(24): fleetdev-secondary-site
[✗] Evaluation fleet.biceptest: fleetShape (fleet.bicep) Failed at 6 / 8 assertions!
	...
	[✗] Assertion primarySiteIsNamedForItsRole failed!
		...
		Could not be evaluated: The input "environment" has no value. Supply it from a test case or give it a default.
	[✗] Assertion eachAccountDeploysToItsRegion failed!
		Each data account must be deployed to the region it was created for.
		Could not be evaluated: The input "environment" has no value. Supply it from a test case or give it a default.
	[✗] Assertion eachSiteRunsOnItsOwnPlan failed!
		Each site must run on the plan of its own stamp.
		Could not be evaluated: The input "environment" has no value. Supply it from a test case or give it a default.
...
- Evaluated values cover resource instances, what each instance declares, and outputs. Values Azure assigns at deployment time are not computed; mock them.
- Reading any evaluated body fails if one instance in scope has a body that cannot be computed offline, even if the assertion is about a different instance.
